...
import {
  ...
  CheckSquare, FolderKanban, GitBranch, Bell, Webhook, Clock,
...
export function WorkflowBuilderPage() {
  ...
  useEffect(() => {
    ...
    workflowsApi.getOne(id!).then(wf => {
      ...
      setActions((wf.actions ?? []).filter(a => !a.parentActionId).map(a => ({
        tempId: a.id,
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    setActions(prev => {
      const oldIdx = prev.findIndex(a => a.tempId === active.id);
      const newIdx = prev.findIndex(a => a.tempId === over.id);
      return arrayMove(prev, oldIdx, newIdx).map((a, i) => ({ ...a, sortOrder: i }));
    });
  };
            ...
            <div className="pt-10 pb-40 w-[480px]">
              <div className="space-y-3">
                ...
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                  <SortableContext items={actions.map(a => a.tempId)} strategy={verticalListSortingStrategy}>
                    <div className="space-y-3">
