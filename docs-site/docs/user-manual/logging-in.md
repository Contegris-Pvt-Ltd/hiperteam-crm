...
sidebar_position: 2
title: "Logging In"
description: "Learn how to log in to Intellicon CRM, manage your session, reset your password, and accept team invitations."
---

# Logging In

Intellicon CRM uses a secure, multi-tenant authentication system. Each organization has its own isolated workspace, and you need valid credentials to access yours.

## The Login Page

When you navigate to your Intellicon CRM URL, you will see the login page with the following fields:
...
Intellicon CRM is multi-tenant, meaning many organizations share the same platform but each has completely isolated data. Your **tenant slug** is a unique short name that identifies your organization — for example, `acme-corp` or `globex`. Your administrator will provide this when setting up your account.
:::

## Multi-Tenant Login

If you belong to multiple organizations on Intellicon CRM, you can log in to each by changing the tenant slug on the login page. Your email and password may differ between tenants, as each workspace manages its own user accounts independently.
...
4. Check your inbox for an email from Intellicon CRM containing a reset link.
...
When an administrator invites you to join their Intellicon CRM workspace:
...
Intellicon CRM uses JSON Web Tokens (JWT) for authentication. Here is what you should know:
